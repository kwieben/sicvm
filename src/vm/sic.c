...
int run(int iExecutionType)
{
	...
	do {
		...
		default:
			...
			break; //REG_A bit of defensive programming ;-)
		}
		/*if (i > 10000)
		  break;*/
...
void execute(unsigned int uiInstructionType, unsigned int uiHexOpcode, unsigned int uiHexOperand)
{
	...
	case ARITHMETIC:
		...
		switch (uiHexOpcode) {
		case ADD:
			REG_A += fetch(DATA_FETCH); //Type 3 Fetch: Fetch Data from memory
			if (REG_A > 0xFFFFFF) {
				flags.hasInterrupt = 1;
		...
		case DIV:
			...
			if (temp == 0) {
				...
				REG_A = (REG_A / temp); // overflow error fixed
			}
			break;
		}
		break;
	...
	case LOGIC:
		...
		case COMP:
			SW = SW & CLEAR;
			if (REG_A == (temp = fetch(DATA_FETCH))) {
				SW = SW | EQ;
			}
			else {
				if (REG_A > temp) {
	...
	case LOADSTORE:
		...
		case STCH:
			ucrgMemory[uiHexOperand] = REG_A & 0xFF;
			break;

		case LDCH:
			REG_A = REG_A & 0xFFFF00;
			REG_A = REG_A | ucrgMemory[uiHexOperand];
			break;
		}
		break;
	...
	case IO:
		...
		case RD:
			uiHexOperand = fetch(DEVCODE_FETCH); //Type 3 (Byte Fetching)
			REG_A = readDevice(fpInputDevice);
		...
		case WD:
			...
			writeDevice(fpOutputDevice, REG_A);
			break;
		}
	}
}
...
int fetch(unsigned int uiFetchType)
{
	...
	case DATA_FETCH:        //Type 0 Data Fetch
